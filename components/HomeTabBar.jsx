      ...
      <div className='flex items-center gap-3 text-sm font-semibold'>
        {productType.map((item) => (
          <button
            ...
            className={`border border-shop_light_green/30 px-4 py-1.5 md:px-6 md:py-2 rounded-full hoverEffect
              ${selectedTab === item.title
                ? "bg-shop_light_green text-white"
                : "bg-shop_light_green/20 hover:bg-shop_light_green hover:text-white"
              }`}
          >
            {item.title}
          </button>
